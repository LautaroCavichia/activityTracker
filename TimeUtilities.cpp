...
namespace TimeUtilities {
    ...
    chrono::system_clock::time_point stringToTimePoint(const string &timeString) {
        ...
        return chrono::system_clock::from_time_t(mktime(&time)) -
               chrono::minutes(60); // Convert tm to time_point and subtract 1 hour (offset)
    }
    ...
    bool isEndAfterStart(const string &startTimeString, const string &endTimeString) {
        ...
        if ((startTime_tm.tm_year != endTime_tm.tm_year) || (startTime_tm.tm_mon != endTime_tm.tm_mon) ||
            (startTime_tm.tm_mday != endTime_tm.tm_mday) || (startTime_t > endTime_t))
            return false;
        else
            return true;
    }
    ...
    string timePointToString(const chrono::system_clock::time_point &timePoint, bool onlyTime) {
